...
body_img_path_map = {
    ...
    "head" : "./shadow_play_material/head2.png",
...
def append_img_by_sk_points_optimized(img, body_part_key, key_point_y, first_point, second_point, 
    ...
    if start_y < end_y and start_x < end_x:
        ...
        if len(region.shape) == 3 and region.shape[2] == 3:
            ...
            # 方法2：如果你的图片有透明通道，可以使用这个
            if region.shape[2] == 4:  # RGBA
                mask = region[:,:,3] > 128  # 透明度大于128的像素
            
            # 方法3：如果你想显示所有非黑色像素，可以使用这个
            # black_threshold = 50
            # mask = (region[:,:,0] > black_threshold) | \
            #        (region[:,:,1] > black_threshold) | \
            #        (region[:,:,2] > black_threshold)
